...
import {SturdyUSDCOracle} from "../periphery/SturdyUSDCOracle.sol";

contract SturdyUSDCOracleTest is Setup {
    SturdyLenderFactory public sturdyLenderFactory =
        new SturdyLenderFactory(management, performanceFeeRecipient, keeper);
    SturdyUSDCOracle public oracle;

    function setUp() public override {
        super.setUp();
        // crvusd vault
        vault = 0x8670120c32DE7bc990e0FE3Bbd04704E98492F0a;

        asset = ERC20(address(IStrategyInterface(vault).asset()));

        strategy = IStrategyInterface(
            0xf7F72e473F01da378E83D9376e07f7225a3945c5
        );

        oracle = new SturdyUSDCOracle();
    }
    ...
    function test_oracle(uint256 _amount, uint16 _percentChange) public {
        vm.assume(_amount > 10_000e6 && _amount < 1_000_000e6);
